...
from source.vector_database import create_user_db, create_user_history_db
...
def decompose_llm(query, llm=llm_openai): 
    ...
    chat_prompt = ChatPromptTemplate(
        [("system", """You are a helpful assistant that splits a user's complex query into a list of simple, self-contained questions and the content that the user want,
        in this case, it can be a concept mentioned. 
            ...
            what is the fundamental Components of LangChain"
            Assistant:
            [
              What is a butterfly?,
              butterfly is,
              ...
              Hi, my name is Isa,
              LangChain fundamental components,
            ]
            ### EXAMPLE ###
...
def get_app():
    ...
    graph.add_node("responseLLM", response_agent)
    graph.set_entry_point("responseLLM")
    graph.add_edge("responseLLM", END)
